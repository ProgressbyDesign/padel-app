...
export default async function PlayerDashboard({account}: {account:{id:string;fullName:string|null;email:string}}) {
    ...
    <div className="grid gap-8 lg:grid-cols-[220px_minmax(0,1fr)]">
      ...
      <div className="min-w-0 space-y-7">
        <section className="relative overflow-hidden rounded-3xl bg-primary p-7 text-white sm:p-9">
          <Image src="/images/hero-padel-overlay.jpg" alt="" fill sizes="900px" className="object-cover opacity-25" />
          <div className="relative flex flex-wrap items-center gap-5"><AccountAvatar url={nav?.avatarUrl} name={account.fullName} email={account.email} size="lg" /><div><h1 className="text-3xl text-white sm:text-4xl">Welcome{account.fullName ? `, ${account.fullName}` : " back"}</h1><p className="mt-3 text-sm text-white/80">Your next session starts here.</p><Link href="/account/settings" className="mt-3 inline-block text-xs underline text-white/80">Edit player profile</Link></div></div>
